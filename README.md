# 🎙️ AI Interview Assistant

An intelligent **voice-based interview assistant** built with **Python**, **OpenAI GPT**, and **real-time speech recognition**.

This app listens to your voice, converts it into text using OpenAI Whisper, and generates human-like answers using GPT.  

---

## 🚀 Features

- 🎤 Real-time microphone input (no need to upload files)
- 🧠 Smart answers powered by **OpenAI GPT-4o-mini**
- 🔊 Optional text-to-speech (AI speaks responses)
- 🪟 Simple GUI using **Tkinter**
- 💡 Easy to extend for new features (e.g., chat memory, topic filters)

---

## ⚙️ Installation & Setup

1. **Clone the repository**
2. **Create a virtual environment**
3. **Install dependencies**
4. **Set your OpenAI API key**
5. **Run the application**



